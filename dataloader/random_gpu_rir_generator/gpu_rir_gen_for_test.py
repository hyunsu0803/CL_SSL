...
class acoustic_simulator_on_the_fly(simulator_common):
    ...
    def create_param(self, num_spk, with_coherent_noise, room_config, mic_type, mic_num):
    
        room_sz=room_config['room_sz']         
        rt60=room_config['Tmax']

        
        if mic_type=='whole' or mic_num=='whole':
            mic=self.whole_mic_setup
        else:
            mic=self.whole_mic_setup
            mic_pos=self.mic_pos_dict[mic_type][mic_num]
            mic['mic_pos']=np.array(mic_pos)
        ...
        self.gpu_rir_param(room_sz, rt60, np.ones(6))
    ...
    def create_rir(self,room_config, num_spk=1, with_coherent_noise=True, mic_type='miyungpa', mic_num=4): 
        
        self.params, mic_pos, azi_list, linear_azi_list=self.create_param(num_spk, with_coherent_noise, room_config, mic_type, mic_num)

        rirs = gpuRIR.simulateRIR(**self.params)    

        return rirs, mic_pos, azi_list, linear_azi_list
