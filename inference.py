...
class Dataloader_config():
    ...
    def config(self):
        self.test_loader_synth = Synth_dataload(self.args['dataloader']['test']['loader'])
        self.test_loader_real = Real_dataload(self.args['dataloader']['test']['loader'])
...
class Tester():
    ...
    def test_SD(self, epoch):
        ...
        for room_type in self.args['hyparam']['result_folder']['room_type']:
            room_type='evaluation_synth'    
            self.dataloader.test_loader_synth.dataset.room_type=str(room_type)

            with torch.no_grad():

                for iter_num, (mixed, vad, speech_azi, white_snr, coherent_snr, rt60) in enumerate(tqdm(self.dataloader.test_loader_synth, desc='Test', total=len(self.dataloader.test_loader_synth))):
                    ...
                    pkl_idx = self.dataloader.test_loader_synth.dataset.pkl_list[iter_num]
    ...
    def test_RD(self, epoch):
        ...
        for room_type in self.args['hyparam']['result_folder']['room_type']:
            room_type='evaluation_real'    
            self.dataloader.test_loader_real.dataset.room_type=str(room_type)

            with torch.no_grad():

                for iter_num, (mixed, vad, pseudo_target, white_snr, coherent_snr, rt60) in enumerate(tqdm(self.dataloader.test_loader_real, desc='Test', total=len(self.dataloader.test_loader_real))):
                    ...
                    pkl_idx = self.dataloader.test_loader_real.dataset.pkl_list[iter_num]
