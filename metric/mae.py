...
def get_pw_losses(loss_func, est_targets, targets, **kwargs):
    ...
    batch_size, n_src, *_ = targets.shape   # (9, 1)
...
def get_doa_error(est_targets, targets):
    # print(est_targets.shape)      # 9
    # print(targets.shape)          # 9 -> 144
    # exit()

    doa_error= torch.abs(est_targets-targets)**2        
    doa_error_360=torch.abs(360-doa_error)**2
...
def calc_rmsae(output, target, vad, num_spk, azimuth, resolution=1, local_maximum_distance=30, calc_layer=[0, 1, 2], acc_threshold=5, ref_vec=0):
    ...
    # print(vad.shape)      # (1, 1, 501)
    output=output.numpy()   # (1, 3, 360, 501)
    # print(azimuth.shape)    # (B, 1)
    azimuth=azimuth.repeat_interleave(len(calc_layer), 0)
    # print(azimuth.shape)    # (3B, 1)
    ...
    for frame_num in range(output.shape[-1]):
        ...
        if np.any(active_spk >= azimuth.shape[1]):
            # print(f"Warning: Some indices in active_spk are out of bounds for azimuth with shape {azimuth.shape}")
            continue
        frame_azimuth=azimuth[:, active_spk]    # (3, 1) probably
        ...
        # print(estimate_frame.shape)         # (9, 1)
    
   
        # print(frame_azimuth.shape)      # (3, 1)
        frame_azimuth=frame_azimuth.repeat_interleave(3,dim=0)
        # print(frame_azimuth.shape)      # (9, 1)  -> all same number
        
        
        # doa_error=get_pw_losses(get_doa_error, estimate_frame, frame_azimuth)   # (9,1)
        doa_error=get_pw_losses(get_doa_error, estimate_frame, frame_azimuth)       # squared error
        acc=doa_error<=acc_threshold

        acc=acc.astype(int).mean(axis=1)
        doa_error=doa_error.sum(axis=-1)    # (9, )
        
        total_argmax_acc += acc[:len(calc_layer)]
        total_softmax_acc += acc[len(calc_layer):2*len(calc_layer)]
        total_half_softmax_acc += acc[-len(calc_layer):]
        

        total_argmax_doa_error += doa_error[:len(calc_layer)]
        total_softmax_doa_error += doa_error[len(calc_layer):2*len(calc_layer)]
        total_half_softmax_doa_error += doa_error[-len(calc_layer):]
...
#         frame_azimuth=azimuth[:, active_spk, frame_num]
