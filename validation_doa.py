...
class Logger_config():
    ...
    def select_best_model(self, DB_type, epoch, model):

        now_dict=self.save_config_dict[DB_type]
        softmax_acc = now_dict['softmax_acc']
        softmax_doa_error = now_dict['softmax_doa_error']
        num = now_dict['number_of_degrees']

        acc = softmax_acc[1]/num
        mae = softmax_doa_error[1]/num
        ...
        self.save_best_mae = False
        if mae < self.best_mae:
            self.save_best_mae = True
            self.best_mae = mae
        ...
        print('MAE: {:.2f}, Acc: {:.2f}'.format(mae, acc*100))
        print('MAE epoch: {}, Acc epoch: {}'.format(epoch, epoch))
        print('Best MAE: {:.2f}, Best Acc: {:.2f}'.format(self.best_mae, self.best_acc*100))
        print('Best MAE epoch: {}, Best Acc epoch: {}'.format(self.best_mae_epoch, self.best_acc_epoch))
...
class Tester():
    ...
    def run(self, ):
        ...
        for epoch in range(149, -1, -1):
            print('Epoch: {}'.format(epoch))

            self.args['hyparam']['model_for_finetune'] = './results/model_checkpoint/epoch_{}.tar'.format(epoch)
            
            del self.learner.model
            self.learner.model_select()
            self.model = self.learner.model

            self.logger.config()
            self.test_RD(epoch)

            if self.logger.save_best_acc:
                # torch.save(self.logger.best_acc_model, "./results/model_checkpoint/best_acc_model.tar")
                print('Best Acc model saved!')
            if self.logger.save_best_mae:
                # torch.save(self.logger.best_mae_model, "./results/model_checkpoint/best_mae_model.tar")
                print('Best MAE model saved!')

        print('Best MAE: {}, Best Acc: {}'.format(self.logger.best_mae, self.logger.best_acc))
        print('Best MAE epoch: {}, Best Acc epoch: {}'.format(self.logger.best_mae_epoch, self.logger.best_acc_epoch))
    ...
    def test_RD(self, epoch):
        ...
        for room_type in self.args['hyparam']['result_folder']['room_type']:
            ...
            with torch.no_grad():
                ...
                self.logger.select_best_model(room_type, epoch, self.model,)
